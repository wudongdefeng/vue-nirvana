          ...
          <div class="box">
            <div class="detail">
              <div class="header-title">
                <span class="title">{{ capuccilo.title }}</span>
              </div>
              <div>
                <!-- <span class="subtitle-item">{{ capuccilo.area }}</span>
                <span class="subtitle-item dot">{{
                  capuccilo.updateTime
                }}</span> -->
                <span
              ...
              <div class="updateTime">
                <span>{{ capuccilo.updateTime }}</span>
              </div>
            </div>
            <div class="pic">
              <div
                class="cover"
                :style="
                  'background-image: url(' + capuccilo.coverImageUrl + ');'
                "
              />
            </div>
          </div>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      this.capuccilo.title = $("h1.title").text();
      store.dispatch("bar/setTitle", this.capuccilo.title + " - 卡布奇洛");
      this.capuccilo.updateTime = $(".data").eq(3).text();
      // this.capuccilo.area = $(".data").eq(0).text();
      let subtitleArr = [];
      ...
      this.capuccilo.introduction = $(".detail-content").text();
      this.capuccilo.coverImageUrl = $(".stui-content__thumb img").attr("src");
      ...
      window.setTimeout(async () => {
        if (window.fy_bridge_app) {
          ...
          history = [
            {
              id: this.detailsId,
              title: this.capuccilo.title.trim(),
              url: url,
              text: text.trim(),
              img: this.capuccilo.coverImageUrl,
              source: "涅槃.卡布奇洛",
          ...
          this.episodes.forEach((e) => {
            ...
            let hikerHistoryMethods = `let history=fetch("hiker://files/nirvana/nirvana_xx_history",{});if(history=='null'||history==null||history=='undefined'||history==undefined||history=='')history=[];else history=JSON.parse(history);history.forEach((h,index)=>{if(h.id=="${
              ...
              this.detailsId
            }",title:"${this.capuccilo.title.trim()}",url:"${
              e.url
            }",text:"${e.text.trim()}",img:"${
              this.capuccilo.coverImageUrl
            ...
            if (e.text.trim() == text.trim()) {
              playUrlArr.push({
                title: this.capuccilo.title.trim() + " - " + e.text.trim(),
                url: playUrl,
                use: true,
                codeAndHeader: ";get",
              });
