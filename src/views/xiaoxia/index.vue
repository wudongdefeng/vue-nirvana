      ...
      window.setTimeout(async () => {
        if (window.fy_bridge_app) {
          ...
          this.episodes.forEach((e) => {
            ...
            let hikerHistoryMethods = `let history=fetch("hiker://files/nirvana/nirvana_xx_history",{});if(history=='null'||history==null||history=='undefined'||history==undefined||history=='')history=[];else history=JSON.parse(history);history.forEach((h,index)=>{if(h.id=="${this.detailsId}")history.splice(index,1)});history.unshift({id:"${this.detailsId}",title:"${this.mengmaindata.title.trim()}",url:"${e.url}",text:"${e.text.trim()}",img:"${this.mengmaindata.coverImageUrl}",source:"涅槃.小虾",time:Math.round(new Date()/1000),isPlayUrl:false,});if(history.length>100)history.splice(100,1);history=JSON.stringify(history);writeFile("hiker://files/nirvana/nirvana_xx_history",history);`;
            if (e.text == text) {
              playUrlArr.push({
